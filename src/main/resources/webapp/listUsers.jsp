...
<body>
	...
	<div class="tabs">
		...
		<span class="menu active"><a href="listUsers.jsp">Users &amp; Sessions</a></span>
		<span class="menu"><a href="traffic.jsp">SSO Traffic</a></span>
				...
				<td valign="top">
					...
					<table>
						<c:forEach items="${requestScope.config.userManager.users}" var="user">
							<tr>
								<td>
									<c:if test="${user.username == requestScope.selectedUserName}">
										<IMG src="pointer.png" />
									</c:if>
								</td>
		...
		<table>
			<c:forEach items="${requestScope.config.sessionManager.cookieDomains}" var="domain">
				...
				<c:forEach items="${jsputils.domainSessions[domain]}" var="session">
					<tr>
						<td><c:if test="${session.token == requestScope.currentToken}">
								<IMG src="pointer.png" />
							</c:if></td>
						<td><a href="/admin/action/set-session/${session.token}">${session.token}</a></td>
						<td>${session.remainingSeconds}</td>
						<td><a href="/admin/action/terminate-session/${session.token}"><img src="delete.gif" style="border: none" /></a></td>
					</tr>
				</c:forEach>
			</c:forEach>
		</table>
