 ...
 * for an application that can be hit through the reverse proxy on a port on 
 * the same box as the proxy.
 * 
 * @author Mark Boyd
 * @copyright: Copyright, 2009, The Church of Jesus Christ of Latter Day Saints
 *
 */
public class AppEndPoint implements Comparable<AppEndPoint>{
	...
	public AppEndPoint(String canonicalCtx, String appCtx, String host, int port) {
		...
		this.host = host;
		this.id = canonicalCtx + "->" + host + ":" + port + appCtx;
	}
	
	public int compareTo(AppEndPoint o) {
	 ...
	 * 
	 * canonical space URL prefix: /mls/mbr
	 * application space URL prefix: /mls-membership
	 * 
	...
	public RequestLine getAppRequestUri(HttpPackage reqPkg) {
		...
		StartLine appReqLn = new StartLine(reqPkg.requestLine.getMethod(),
				applicationContextRoot + reqPkg.requestLine.getUri().substring(canonicalContextRoot.length()),
				reqPkg.requestLine.getHttpDecl());
		//reqPkg.headerBfr.append("cctx: " + canonicalContextRoot);
		
		/* versions prior to clientlib 1.1.4 used query param not header
		if (appReqLn.token2.indexOf('?') == -1) {
			// no query string, append one
			appReqLn.token2 += "?" + Config.CANONICAL_CTX_QPARAM_NAME + "=" + canonicalContextRoot;
		}
		else {
			// query string found, append to existing
			appReqLn.token2 += "&" + Config.CANONICAL_CTX_QPARAM_NAME + "=" + canonicalContextRoot;
		}
		*/
		return appReqLn;
